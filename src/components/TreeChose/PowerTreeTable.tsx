...
export default function TreeTable(props: Props): JSX.Element {
  ...
  const [btnDtoChecked, setBtnDtoChecked] = useState<string[]>([]); // 受控，所有被选中的权限数据
  ...
  const sourceData = useMemo(() => {
    return dataToJson(null, props.data) || [];
  ...
  const tableRowSelection:any = useMemo(() => {
    return {
      onChange: (selectedRowKeys: string[]): void => {
        setTreeChecked(selectedRowKeys);
      },
      onSelect: (record: TableData, selected: boolean): void => {
        const t:any = props.data.find((item:any) => item.id === record.id);
        if (selected) {
          // 选中，连带其权限全部勾选
          if (t && Array.isArray(t.powers)) {
            const temp = Array.from(
              new Set([...t.powers.map((item:any) => item.id), ...btnDtoChecked])
            );
          ...
          // 取消选中，连带其权限全部取消勾选
          if (t && Array.isArray(t.powers)) {
            const mapTemp = t.powers.map((item:any) => item.id);
      ...
      onSelectAll: (selected: boolean) => {
        if (selected) {
          // 选中
          setBtnDtoChecked(
            props.data.reduce((v1:any, v2:any) => {
              return [...v1, ...v2.powers.map((k:any) => k.id)];
            }, [])
          );
        } else {
          setBtnDtoChecked([]);
