...
function ArticleHeaderList(props:any) {
  ...
  const getList = () => {
    axios.get(servicePath.getHeaderInfo).then((res) => {
      setList(res.data)
    })
  }
  //删除文章的方法
  const delHeaderType = (id:string) => {
    confirm({
      ...
      onOk() {
        axios(servicePath.delHeaderType + id).then(
          (res) => {
  ...
  const handleOk = () => {
    form.validateFields().then((values:records) => {
      if (isEdit) {
        values.id = singleRecord.id
        axios.post(servicePath.updateHeaderType, values).then(
          (res) => {
            getList()
            setVisible(false)
            setIsEdit(false)
        ...
        axios.post(servicePath.addHeaderType, values).then(
          (res) => {
            getList()
            setVisible(false)
            setIsEdit(false)
  ...
  //修改文章
  const updateHeaderType = (record:any) => {
    setVisible(true)
    setIsEdit(true)
  ...
  return (
    <div>
      ...
      <Table dataSource={list} rowKey="id">
        <Column title="标签" render={(text, record:records) => <Tag color="blue">{record.typeName}</Tag>}></Column>
        <Column title="路由" dataIndex="route" />
