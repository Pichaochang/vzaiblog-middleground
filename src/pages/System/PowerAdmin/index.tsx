...
function PowerAdminContainer() {
  ...
  const getData = async (menuId: string | number | null = null) => {
    ...
    const res: Res = await axios.post(servicePath.getPowerByMenuId, params)
  ...
  // 点击树目录时触发
  const onTreeSelect: any = (
    keys: React.ReactText[],
    info: {
      event: "select";
      selected: boolean;
      node: EventDataNode & { id: string; title: string };
  ...
  const onModalShow = (data: TableRecordData | null, type: operateType) => {
    ...
    //           (item2: any) => item2.menuId === data.menuId
    //         );
    //         if (theMenuPower) {
  ...
  const onOk = async () => {
    ...
    if (modal.operateType === "add") {
      ...
      const res: Res = await axios.post(servicePath.updatePower, params);
      if (res && res.code === 200) {
  ...
  // 删除一条数据
  const onDel: any = async (record: TableRecordData) => {
    const params = { id: record.id };
    setLoading(true);
    const res: any = await axios.post(servicePath.deletePower, params);
    if (res && res.code === 200) {
      getData(treeSelect.id);
      // dispatch.app.updateUserInfo();
  ...
  const sourceData = useMemo(() => {
    const d: Menu[] = cloneDeep(menus); // @todo
    d.forEach((item: any) => {
          ...
          {/* <Form.Item label="赋予" {...formItemLayout}>
            <Checkbox.Group
              ...
              onChange={(v: CheckboxValueType[]) =>
                setRolesCheckboxChose(v as string[])
              }
            />
          </Form.Item> */}
        </Form>
